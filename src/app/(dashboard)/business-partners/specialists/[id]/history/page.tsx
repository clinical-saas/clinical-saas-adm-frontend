...
export default function SpecialistHistoryPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: queryKeys.specialists.history(id),
    queryFn: () =>
      apiClient<ServiceHistoryEntry[]>(`/service-provider/${id}/history`),
  });
      ...
      <PageHeader
        ...
        action={
          <Link href={`/business-partners/specialists/${id}`}>
            <Button variant="outline">Back to Specialist</Button>
          </Link>
        }
      />
      {data && data.length > 0 ? (
        ...
        <EmptyState
          ...
          description="This specialist has no service history yet."
        />
      )}
    </div>
  );
